...
function renderAutomovil() {
  ...
  for (let i = 0; i < automovil.length; i++) {
    html =
      html +
      `
	  <div onclick="addToCart(${automovil[i].id});saveToLocalStorage();"style="border: 1px solid green;margin: 10px;">
...
const desestructurar = ( {price} ) => {
  console.log(price)
}
desestructurar(cart);

const cliente = (cart.length==0) ? true : false
cliente ? console.log("Aun no ha comprado") : console.log("Ya es un cliente");
